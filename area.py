...
def generate_raw_data(path_txt,txt_name):
    ...
    for j in range(areaNum):
        ...
        if ((na=="Agalg") or (na=="Aga") or (na=="Agaf") or (na=="Col") or (na=="Helc") or (na=="Lep") or (na=="Mdrc") or (na=="Mdrsa") or (na=="Mdrcb") or (na=="Mdrsd") or (na=="Mdrsf") or (na=="Mdrsp") or (na=="Man") or (na=="Mon") or (na=="Ocu") or (na=="Sco") or (na=="Sol") or (na=="Ste") or (na=="STY")):
            ...
            txtfile.close()
    txtfile.close()
    with open(str('2013/2013new/')+txt_name+str('.txt'), 'w+') as newtxtfile:
...
txt_name = glob.glob("./2013/2013area/*.txt")
count = 1
for name in txt_name:
    image_name = name.split("\\")[1]
    image_name = image_name.split(".")[0]
    path_txt = str('./2013/2013area/')+image_name+str('.txt')
...
def generate_all_data(new_txt_name):
    ...
    new_path_txt = str('./2013/2013area_original - Copy/')+new_txt_name+ str('.txt')
...
#%%
new_txt_name = glob.glob("./2013/2013area/*.txt")
all_data_count = 1
for name in new_txt_name[106:]:
    image_name = name.split("\\")[1]
    image_name = image_name.split(".")[0]
...
def read_generate_data(txtfolder_name,imagefloder_name):
    ...
    read_files = glob.glob(str('./2013/')+txtfolder_name+str('/*.txt'))
    for name in read_files:
        name = name.split("\\")[1]
        name = name.split(".")[0]
        path_txt = str('./2013/')+txtfolder_name+str('/')+name+str('.txt')
        path_image = str('./2013/')+ imagefloder_name+str('/')+name+str('.jpg')
...
#%%read new data
imagefloder_name = '2013image'
new_txtfolder_name = '2013area_original - Copy'
new_final_data,new_final_label = read_generate_data(new_txtfolder_name,imagefloder_name)
#%%read previous data
old_txtfolder_name = '2013new - Copy'
old_final_data,old_final_label = read_generate_data(old_txtfolder_name,imagefloder_name)
#%%
final_data = np.vstack((new_final_data,old_final_data))
final_label = np.hstack((new_final_label,old_final_label))
#%%
