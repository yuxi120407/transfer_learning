...
def newtxt(path_txt,path_image,crop_length,crop_width):
    ...
    for m in range(count_points):
        ...
        if ((new_l=="Agalg") or (new_l=="Aga") or (new_l=="Agaf") or (new_l=="Col") or (new_l=="Lep") or (new_l=="Helc") or (new_l=="Mdrc") or (new_l=="Mdrsa") or (new_l=="Mdrcb") or (new_l=="Mdrsd")or (new_l=="Mdrsf")or (new_l=="Mdrsp") or (new_l=="Man")or (new_l=="Mon")or (new_l=="Ocu")or (new_l=="Sco")or (new_l=="Sol")or (new_l=="Ste")or (new_l=="STY")):
            label_encode[m] = 0
        elif (new_l=="DCP"):
            label_encode[m] = 1
        elif (new_l=="ROC"):
    ...
    name_image = path_image.split("/")[3] 
    name_txt = name_image.split(".")[0]
    with open(str('2013/2013new/')+name_txt+str('.txt'), 'w+') as newtxtfile:
...
def newimagedata(crop_length,crop_width):
    ...
    read_files = glob.glob("./2013/2013image/*.jpg")
    #length = len(read_files)
    for name in read_files:
        name = name.split("\\")[1]
        name = name.split(".")[0]
        path_txt = str('./2013/2013new/')+name+str('.txt')
        path_image = str('./2013/2013image/')+name+str('.jpg')
...
def transfer_model():
    ...
    #for layer in new_model.layers[0:22]:
    ...
    for i in range(19):
        ...
        new_model.layers[i].set_weights(model_weight)
    #new_model.load_weights('./model_weight/2012images-areas-7.5-50epoch.h5')
